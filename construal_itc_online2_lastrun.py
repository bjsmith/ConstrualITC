...
expName = 'construal_itc_online2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/benjaminsmith/GDrive/neural-construal-level/code/SmithAdaptConstrualITC/construal_itc_online2.psyexp',
...
Img_right_feedback = visual.ImageStim(
    win=win, name='Img_right_feedback',
    image='images/blue-highlight-bg.png', mask=None,
    ori=0, pos=(0.5, 0), size=(0.3, 0.2),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
Img_left_feedback = visual.ImageStim(
    win=win, name='Img_left_feedback',
    image='images/red-highlight-bg.png', mask=None,
    ori=0, pos=(-0.5, 0), size=(0.3,0.2),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
ImgChoice2Background = visual.ImageStim(
    ...
    image='images/bluebg.png', mask=None,
    ori=0, pos=(0.5, 0), size=(0.3, 0.2),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
text_3 = visual.TextStim(win=win, name='text_3',
    text="We'll show you another block now.\n\nbe good!",
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0);
...
fixedDelayTrials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('testdesign-fixedDelay.csv'),
    seed=None, name='fixedDelayTrials')
thisExp.addLoop(fixedDelayTrials)  # add the loop to the experiment
thisFixedDelayTrial = fixedDelayTrials.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisFixedDelayTrial.rgb)
if thisFixedDelayTrial != None:
    for paramName in thisFixedDelayTrial.keys():
        exec(paramName + '= thisFixedDelayTrial.' + paramName)

for thisFixedDelayTrial in fixedDelayTrials:
    currentLoop = fixedDelayTrials
    # abbreviate parameter names if possible (e.g. rgb = thisFixedDelayTrial.rgb)
    if thisFixedDelayTrial != None:
        for paramName in thisFixedDelayTrial.keys():
            exec(paramName + '= thisFixedDelayTrial.' + paramName)
    ...
    continueRoutine = True
    # update component parameters for each repeat
    ImgChoice1Background.setImage('images/redbg.png')
    Delay1.setColor('white', colorSpace='rgb')
    Delay1.setText(SSdelay)
    Delay1.setOri(0)
    ...
    # keep track of which components have finished
    itc_trialComponents = [Img_right_feedback, Img_left_feedback, ImgChoice2Background, ImgChoice1Background, Delay1, Delay2, Amount1, Amount2, resp, no_response, text_middle_line]
    ...
    while continueRoutine:
        ...
        # *Img_right_feedback* updates
        if ((False)) and Img_right_feedback.status == NOT_STARTED:
            # keep track of start time/frame for later
            Img_right_feedback.tStart = t
            Img_right_feedback.frameNStart = frameN  # exact frame index
            Img_right_feedback.setAutoDraw(True)
        
        # *Img_left_feedback* updates
        if ((False)) and Img_left_feedback.status == NOT_STARTED:
            # keep track of start time/frame for later
            Img_left_feedback.tStart = t
            Img_left_feedback.frameNStart = frameN  # exact frame index
            Img_left_feedback.setAutoDraw(True)
    ...
    # store data for fixedDelayTrials (TrialHandler)
    fixedDelayTrials.addData('resp.keys',resp.keys)
    fixedDelayTrials.addData('resp.corr', resp.corr)
    if resp.keys != None:  # we had a response
        fixedDelayTrials.addData('resp.rt', resp.rt)
...
# completed 1.0 repeats of 'fixedDelayTrials'

# get names of stimulus parameters
if fixedDelayTrials.trialList in ([], [None], None):
    params = []
else:
    params = fixedDelayTrials.trialList[0].keys()
# save data for this loop
fixedDelayTrials.saveAsExcel(filename + '.xlsx', sheetName='fixedDelayTrials',
    stimOut=params,
...
# set up handler to look after randomisation of conditions etc
fixedAmountTrials = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('testdesign-fixedAmount.csv'),
    seed=None, name='fixedAmountTrials')
thisExp.addLoop(fixedAmountTrials)  # add the loop to the experiment
thisFixedAmountTrial = fixedAmountTrials.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisFixedAmountTrial.rgb)
if thisFixedAmountTrial != None:
    for paramName in thisFixedAmountTrial.keys():
        exec(paramName + '= thisFixedAmountTrial.' + paramName)

for thisFixedAmountTrial in fixedAmountTrials:
    currentLoop = fixedAmountTrials
    # abbreviate parameter names if possible (e.g. rgb = thisFixedAmountTrial.rgb)
    if thisFixedAmountTrial != None:
        for paramName in thisFixedAmountTrial.keys():
            exec(paramName + '= thisFixedAmountTrial.' + paramName)
    ...
    if resp.keys in ['', [], None]:  # No response was made
        ...
        if str('"1"').lower() == 'none':
           ...
           resp.corr = 0  # failed to respond (incorrectly)
    # store data for fixedAmountTrials (TrialHandler)
    fixedAmountTrials.addData('resp.keys',resp.keys)
    fixedAmountTrials.addData('resp.corr', resp.corr)
    if resp.keys != None:  # we had a response
        fixedAmountTrials.addData('resp.rt', resp.rt)
    # the Routine "itc_trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
...
# completed 1 repeats of 'fixedAmountTrials'

# get names of stimulus parameters
if fixedAmountTrials.trialList in ([], [None], None):
    params = []
else:
    params = fixedAmountTrials.trialList[0].keys()
# save data for this loop
fixedAmountTrials.saveAsExcel(filename + '.xlsx', sheetName='fixedAmountTrials',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
...
# update component parameters for each repeat
# keep track of which components have finished
thanksComponents = [thanksText, text_4]
